...
import umontreal.ssj.splitting.old.createGraph;
...
public class TestParams {
   ...
   static void setRel(GraphOld graph) {
   ...
   public static void setRelRand(GraphOld graph) {
      int m = graph.getNumLinks();
   ...
   static void setRelcomp(GraphOld graph) {
      int m = graph.getNumLinks();
      int n = graph.getNumNodes();
      LinkOld[] ar = graph.getLinks();
      double r = .999998;
      for (int i = 0; i < m; i++) {
         LinkOld lin = ar[i];
   ...
   static ShockList getShocks(GraphOld graph, double rateN, double rateL)
         throws IOException {
   ...
   private static ShockList getShocksFromNodes(GraphOld graph, double rate) {
   ...
   private static ShockList getShocksFromLinks(GraphOld graph, double rate) {
   ...
   private static ShockList getShocksFromNodesAndLinks(GraphOld graph,
   ...
   private static ShockList getShocksParallelFromNodes(GraphOld graph, double rate) {
   ...
   static ShockList getShocksBRE(GraphOld graph, double eps) {
      ShockList shocks = ShockList.shocksFromLinksFailure(graph);
   ...
   static void monV0(GraphOld graph, int n) {
   ...
   static void V0dode(GraphOld graph) {
      int[] S2 = { 0, 3, 6, 9, 12, 15, 19 };
      int[] S = { 0, 1, 2, 3, 16, 17, 18, 19 };
      graph.setV0(S);
   }

   public static GraphOld getGraph(String GraphName) throws IOException {
      String file = getGraphfile(GraphName);
      GraphOld graph = new GraphOld(file);
      // monV0 (graph, 10);
      // V0dode (graph);
      return graph;
   }
