 ...
 * A Graph object is defined by a set of {@link NodeOld}'s and a set of
 * {@link LinkOld}'s. We assume that each node is connected with at least one
 ...
 * 
 */
public class GraphOld implements Cloneable {
   /**
    * Number of nodes
    */
    ...
    */
   private LinkOld[] links;

   /**
    * All the nodes of the graph
    */
   private NodeOld[] nodes;
    ...
    */
   public GraphOld(String file) throws IOException {
      ...
      links = new LinkOld[numLinks];
      nodes = new NodeOld[numNodes];
      for (int i = 0; i < numNodes; i++)
         nodes[i] = new NodeOld(i);
    ...
    */
   public LinkOld getLinkFromNode(int j, int i) {
    ...
    */
   public int getNeighborOfNode(LinkOld link, int i) {
      if (link.getSource() == i)
   ...
   @Override
   public GraphOld clone() {
      GraphOld image = null;

      try {
         image = (GraphOld) super.clone();
      ...
      // Link
      image.links = new LinkOld[numLinks];
      for (int i = 0; i < numLinks; i++) {
         image.links[i] = new LinkOld(links[i].getIndice(), links[i].getSource(),
               links[i].getTarget(), links[i].getR());
      }

      // nodes
      image.nodes = new NodeOld[numNodes];
      for (int i = 0; i < numNodes; i++) {
         if (nodes[i].getNodeLinks() != null) {
            ...
            image.nodes[i] = new NodeOld(0, nodes[i].getNumber(), clonemylink);
         } else {
            image.nodes[i] = new NodeOld();
         }
      }
