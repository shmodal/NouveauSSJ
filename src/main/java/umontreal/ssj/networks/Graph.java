...
import umontreal.ssj.networks.old.NodeOld;
...
public class Graph implements Cloneable {
    ...
    */
   private ArrayList<NodeOld> nodes;
   ...
   public Graph(String file) throws IOException {
      ...
      nodes = new ArrayList<NodeOld>();
      for (int i = 0; i < numNodes; i++)
         nodes.add(new NodeOld(i));
    ...
    */
   public ArrayList<NodeOld> getNodes() {
      return nodes;
   }
    ...
    */
   public Link getLinkFromNode(int j, int i) {
    ...
    */
   public int getNeighborOfNode(Link link, int i) {
      if (link.getSource() == i)
   ...
   public Graph clone() {
      ...
      // Link
      image.links = new ArrayList<Link>();
      for (int i = 0; i < numLinks; i++) {
         image.links.add(new LinkCapacity(links.get(i).getIndice(), links.get(i).getSource(),
        		 links.get(i).getTarget(), links.get(i).getR()));
      }

      // nodes
      image.nodes = new ArrayList<NodeOld>();
      for (int i = 0; i < numNodes; i++) {
         if (nodes.get(i).getNodeLinks() != null) {
            int clonemylink[] = new int[nodes.get(i).getNodeLinks().length];
            for (int j = 0; j < clonemylink.length; j++) {
               clonemylink[j] = nodes.get(i).getNodeLink(j);
            }
            image.nodes.add( new NodeOld(0, nodes.get(i).getNumber(), clonemylink));
         } else {
            image.nodes.add(new NodeOld());
         }
      }
