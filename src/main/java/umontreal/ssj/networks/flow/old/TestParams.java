...
public class TestParams {
   ...
   static void setRel(Graph graph) {
   ...
   static void setRelRand(Graph graph) {
      int m = graph.getNumLinks();
   ...
   static void setRelcomp(Graph graph) {
      int m = graph.getNumLinks();
      int n = graph.getNumNodes();
      LinkWithCapacity[] ar = graph.getLinks();
      double r = .999998;
      for (int i = 0; i < m; i++) {
         LinkWithCapacity lin = ar[i];
   ...
   static ShockList getShocks(Graph graph, double rateN, double rateL)
         throws IOException {
   ...
   private static ShockList getShocksFromNodes(Graph graph, double rate) {
   ...
   private static ShockList getShocksFromLinks(Graph graph, double rate) {
   ...
   private static ShockList getShocksFromNodesAndLinks(Graph graph,
   ...
   private static ShockList getShocksParallelFromNodes(Graph graph, double rate) {
   ...
   static ShockList getShocksBRE(Graph graph, double eps) {
      ShockList shocks = ShockList.shocksFromLinksFailure(graph);
   ...
   static void monV0(Graph graph, int n) {
   ...
   static void V0dode(Graph graph) {
      int[] S2 = { 0, 3, 6, 9, 12, 15, 19 };
      int[] S = { 0, 1, 2, 3, 16, 17, 18, 19 };
      graph.setV0(S);
   }

   static Graph getGraph(String GraphName) throws IOException {
